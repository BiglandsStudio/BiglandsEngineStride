...
namespace BiglandsEngine.TestRunner
{
    class TestServerHost : RouterServiceServer
    {
        ...
        public TestServerHost(int bn, string branch) : base("/task/BiglandsEngine.TestRunner.exe")
        {
            buildNumber = bn;
            branchName = branch;
        }
        ...
        public int RunAndroidTest(ConnectedDevice device, bool reinstall, string packageName, string packageFile, string resultFilename)
        {
            ...
            try
            {
                ...
                AddAndroidParameter(parameters, Graphics.Regression.TestRunner.BiglandsEngineVersion, BiglandsEngineVersion.NuGetVersion);
                AddAndroidParameter(parameters, Graphics.Regression.TestRunner.BiglandsEngineBuildNumber, buildNumber.ToString());
                if (!IsNullOrEmpty(branchName))
                    AddAndroidParameter(parameters, Graphics.Regression.TestRunner.BiglandsEngineBranchName, branchName);
    ...
    class Program
    {
        static int Main(string[] args)
        {
            ...
            var p = new OptionSet
            {
                "Copyright (c) .NET Foundation and Contributors (https://dotnetfoundation.org/ & https://BiglandsEngine3d.net) and Silicon Studio Corp. (https://www.siliconstudio.co.jp) All Rights Reserved", "BiglandsEngine Test Suite Tool - Version: " + Format("{0}.{1}.{2}", typeof(Program).Assembly.GetName().Version.Major, typeof(Program).Assembly.GetName().Version.Minor, typeof(Program).Assembly.GetName().Version.Build) + Empty, Format("Usage: {0} [assemblies|apk] -option1 -option2:a", exeName), Empty, "=== Options ===", Empty, { "h|help", "Show this message and exit", v => showHelp = v != null }, { "result-path:", "Result .XML output path", v => resultPath = v }, { "no-reinstall-apk", "Do not reinstall APK", v => reinstall = false },
            };
        ...
        private static int BuildAndRunAndroidTests(List<string> commandArgs, bool reinstall, string resultPath)
        {
            ...
            // get build number
            int buildNumber;
            if (!Int32.TryParse(Environment.GetEnvironmentVariable("BiglandsEngine_BUILD_NUMBER"), out buildNumber))
                buildNumber = -1;

            // get branch name
            var branchName = Environment.GetEnvironmentVariable("BiglandsEngine_BRANCH_NAME");
