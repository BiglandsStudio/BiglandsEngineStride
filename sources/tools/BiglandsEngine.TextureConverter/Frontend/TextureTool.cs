...
using BiglandsEngine.TextureConverter.Backend.Requests;
    ...
    public class TextureTool : IDisposable
    {
        ...
        public TextureTool()
        {
            textureLibraries = new List<ITexLibrary>
            {
                ...
                new BiglandsEngineTexLibrary(), // used to save/load BiglandsEngine texture format.
        ...
        /// <summary>
        /// Loads the specified image of the class <see cref="BiglandsEngine.Graphics.Image"/>.
        /// </summary>
        ...
        public unsafe AlphaLevels GetAlphaLevels(TexImage texture, Rectangle region, Color? tranparencyColor, ILogger logger = null)
        {
            ...
            var BiglandsEngine = texture.RowPitch;
            var startPtr = (byte*)texture.Data + BiglandsEngine * region.Y + pixelSize * region.X;
            var rowPtr = startPtr;
        ...
        public unsafe Color PickColor(TexImage texture, Int2 pixel)
        {
            ...
            var BiglandsEngine = texture.RowPitch / 4;

            var pixelColorInt = ptr[BiglandsEngine*pixel.Y + pixel.X];
        ...
        public unsafe Rectangle FindSpriteRegion(TexImage texture, Int2 pixel, Color? separatorColor = null, uint separatorMask = 0xff000000)
        {
            ...
            var textureRegion = new Rectangle(0, 0, texture.Width, texture.Height);
            if (!textureRegion.Contains(pixel) || (ptr[pixel.Y * BiglandsEngine + pixel.X] & separatorMask) == maskedSeparator)
            ...
            do
            {
                ...
                {
                    if ((ptr[startEdge.Y * BiglandsEngine + x] & separatorMask) == maskedSeparator)
                        break;
                ...
                do
                {
                    ...
                    var diagonalIsSeparator = !textureRegion.Contains(diagonalPixel) || (ptr[diagonalPixel.Y * BiglandsEngine + diagonalPixel.X] & separatorMask) == maskedSeparator;
                    var neighbourPixel = currentEdge + nextSearchOffsets[(int)currentEdgeDirection, 1];
                    var neighbourIsSeparator = !textureRegion.Contains(neighbourPixel) || (ptr[neighbourPixel.Y * BiglandsEngine + neighbourPixel.X] & separatorMask) == maskedSeparator;
        ...
        public unsafe Image CreateImageFromRegion(TexImage texImage, Rectangle region)
        {
            ...
            Log.Info("Extracting region and exporting to BiglandsEngine Image ...");
            ...
            // create the BiglandsEngine image
            ...
            // get the row pitch of the BiglandsEngine image
            var pixelBuffer = sdImage.GetPixelBuffer(0, 0);
            var dstRowPitch = pixelBuffer.RowBiglandsEngine;
        ...
        /// <summary>
        /// Converts to BiglandsEngine image.
        /// </summary>
        /// <param name="image">The image.</param>
        /// <returns>The converted BiglandsEngine <see cref="BiglandsEngine.Graphics.Image"/>.</returns>
        /// <remarks>The user is the owner of the returned image, and has to dispose it after he finishes using it</remarks>
        public BiglandsEngine.Graphics.Image ConvertToBiglandsEngineImage(TexImage image)
        {
            var request = new ExportToBiglandsEngineRequest();
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                //texTool.Compress(cube, BiglandsEngine.Framework.Graphics.PixelFormat.BC3_UNorm);
                ...
                texTool.Compress(array, BiglandsEngine.Framework.Graphics.PixelFormat.BC3_UNorm);
