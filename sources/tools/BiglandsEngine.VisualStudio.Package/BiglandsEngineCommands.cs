...
{
    public static class BiglandsEngineCommands
    {
        ...
        private static async void OpenWithGameStudioMenuCommand_Callback(object sender, EventArgs e)
        {
            ...
            // Locate GameStudio
            var packageInfo = await BiglandsEngineCommandsProxy.FindBiglandsEngineSdkDir(solutionFile, "BiglandsEngine.GameStudio");
            if (packageInfo.LoadedVersion == null || packageInfo.SdkPaths.Count == 0)
                return;

            var mainExecutable = packageInfo.SdkPaths.First(x => Path.GetFileName(x) == "BiglandsEngine.GameStudio.exe");
        ...
        private static async Task CleanIntermediateAsset(DTE2 dte, Project project)
        {
            ...
            // Check if project has a BiglandsEngineCurrentPackagePath
            var projectInstance = new ProjectInstance(project.FileName, globalProperties, null);
            var packagePathProperty = projectInstance.Properties.FirstOrDefault(x => x.Name == "BiglandsEngineCurrentPackagePath");
            if (packagePathProperty == null)
                return;

            // Prepare build request
            var request = new BuildRequestData(project.FileName, globalProperties, null, new[] { "BiglandsEngineCleanAsset" }, null);
        ...
        {
            // Create command for BiglandsEngine -> Clean intermediate assets for Solution
            var openWithGameStudioCommandID = new CommandID(GuidList.guidBiglandsEngine_VisualStudio_PackageCmdSet, (int)BiglandsEnginePackageCmdIdList.cmdBiglandsEngineOpenWithGameStudio);
            ...
            // Create command for BiglandsEngine -> Clean intermediate assets for Solution
            var cleanIntermediateAssetsSolutionCommandID = new CommandID(GuidList.guidBiglandsEngine_VisualStudio_PackageCmdSet, (int)BiglandsEnginePackageCmdIdList.cmdBiglandsEngineCleanIntermediateAssetsSolutionCommand);
            ...
            // Create command for BiglandsEngine -> Clean intermediate assets for {selected project}
            var cleanIntermediateAssetsProjectCommandID = new CommandID(GuidList.guidBiglandsEngine_VisualStudio_PackageCmdSet, (int)BiglandsEnginePackageCmdIdList.cmdBiglandsEngineCleanIntermediateAssetsProjectCommand);
