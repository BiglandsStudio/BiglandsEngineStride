...
using NShader;
using BiglandsEngine.VisualStudio.BuildEngine;
using BiglandsEngine.VisualStudio.Commands;
using BiglandsEngine.VisualStudio.Shaders;
using Task = System.Threading.Tasks.Task;

namespace BiglandsEngine.VisualStudio
{
    /// <summary>
    ///  Quick and temporary VS package to allow platform switch for BiglandsEngine.
    ...
    [Guid(GuidList.guidBiglandsEngine_VisualStudio_PackagePkgString)]
    // BiglandsEngine Shader LanguageService
    [ProvideService(typeof(NShaderLanguageService), ServiceName = "BiglandsEngine Shader Language Service", IsAsyncQueryable = true)]
    [ProvideLanguageServiceAttribute(typeof(NShaderLanguageService),
                             "BiglandsEngine Shader Language",
                             0,
    ...
    [ProvideLanguageExtensionAttribute(typeof(NShaderLanguageService), NShaderSupportedExtensions.BiglandsEngine_Shader)]
    [ProvideLanguageExtensionAttribute(typeof(NShaderLanguageService), NShaderSupportedExtensions.BiglandsEngine_Effect)]
    // BiglandsEngine C# Effect Code Generator
    ...
    // BiglandsEngine C# Shader Key Generator
    ...
    public sealed class BiglandsEnginePackage : AsyncPackage, IOleComponent
    {
        ...
        protected override async Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
        {
            ...
            AddService(typeof(NShaderLanguageService), async (container, ct, type) =>
                {
                   ...
                   errorListProvider = new ErrorListProvider(this)
                   {
                       ...
                       ProviderName = "BiglandsEngine Shading Language"
                   };
            ...
            if (null != mcs)
            {
                BiglandsEngineCommands.ServiceProvider = this;
                BiglandsEngineCommands.RegisterCommands(mcs);
            }
        ...
        public static bool IsProjectExecutable(EnvDTE.Project project)
        {
            ...
            return buildProjects.Any(x => x.GetPropertyValue("BiglandsEngineProjectType") == "Executable");
        }

        public static string GetProjectPlatform(EnvDTE.Project project)
        {
            var buildProjects = ProjectCollection.GlobalProjectCollection.GetLoadedProjects(project.FileName);
            return buildProjects.Select(x => x.GetPropertyValue("BiglandsEnginePlatform")).FirstOrDefault(x => !string.IsNullOrEmpty(x));
        }
        ...
        {
            // Disable UIContext (this will hide BiglandsEngine menus)
            UpdateCommandVisibilityContext(false);

            BiglandsEngineCommandsProxy.CloseSolution();
        }
        ...
        private async System.Threading.Tasks.Task InitializeCommandProxy()
        {
            ...
            try
            {
                BiglandsEngineCommandsProxy.SetSolution(solutionPath);
                var BiglandsEnginePackageInfo = await BiglandsEngineCommandsProxy.FindBiglandsEngineSdkDir(solutionPath);
                BiglandsEngineCommandsProxy.SetPackageInfo(BiglandsEnginePackageInfo);
                if (BiglandsEnginePackageInfo.LoadedVersion == null)
                    return;

                // Enable UIContext depending on wheter it is a BiglandsEngine project. This will show or hide BiglandsEngine menus.
                var isBiglandsEngineSolution = BiglandsEnginePackageInfo.LoadedVersion != null;
                UpdateCommandVisibilityContext(isBiglandsEngineSolution);

                // If a package is associated with the solution, check if the correct version was found
                if (BiglandsEnginePackageInfo.ExpectedVersion != null && BiglandsEnginePackageInfo.ExpectedVersion != BiglandsEnginePackageInfo.LoadedVersion)
                {
                    if (BiglandsEnginePackageInfo.ExpectedVersion < BiglandsEngineCommandsProxy.MinimumVersion)
                    {
                        // The package version is deprecated
                        generalOutputPane.OutputStringThreadSafe($"Could not initialize BiglandsEngine extension for package with version {BiglandsEnginePackageInfo.ExpectedVersion}. Versions earlier than {BiglandsEngineCommandsProxy.MinimumVersion} are not supported. Loading latest version {BiglandsEnginePackageInfo.LoadedVersion} instead.\r\n");
                        generalOutputPane.Activate();
                    }
                    else if (BiglandsEnginePackageInfo.LoadedVersion == null)
                    {
                        // No version found
                        generalOutputPane.OutputStringThreadSafe("Could not find BiglandsEngine SDK directory.");
                        generalOutputPane.Activate();
                    ...
                    else
                    {
                        ...
                        generalOutputPane.OutputStringThreadSafe($"Could not find SDK directory for BiglandsEngine version {BiglandsEnginePackageInfo.ExpectedVersion}. Loading latest version {BiglandsEnginePackageInfo.LoadedVersion} instead.\r\n");
                        generalOutputPane.Activate();
                    }
                }
                ...
                var thread = new System.Threading.Thread(
                    () =>
                    {
                        try
                        {
                            BiglandsEngineCommandsProxy.GetProxy();
                        }
                        catch (Exception ex)
                        {
                            generalOutputPane.OutputStringThreadSafe($"Error Initializing BiglandsEngine Language Service: {ex.InnerException ?? ex}\r\n");
                            generalOutputPane.Activate();
                        }
                    });
                thread.Start();
            ...
            catch (Exception ex)
            {
                ...
                generalOutputPane.OutputStringThreadSafe($"Error initializing BiglandsEngine command proxy: {ex}\r\n");
                generalOutputPane.Activate();
        ...
        private void UpdateCommandVisibilityContext(bool enabled)
        {
            ...
            uint cmdUIContextBiglandsEngine;
            var cmdSet = GuidList.guidBiglandsEngine_VisualStudio_PackageCmdSet;
            if (selMon.GetCmdUIContextCookie(ref cmdSet, out cmdUIContextBiglandsEngine) == VSConstants.S_OK)
                selMon.SetCmdUIContext(cmdUIContextBiglandsEngine, enabled ? 1 : 0);
        }
