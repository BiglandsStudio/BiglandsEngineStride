    ...
    public static class RouterHelper
    {
        ...
        public static bool EnsureRouterLaunched(bool attachChildJob = false, bool checkIfPortOpen = true)
        {
            try
            {
                ...
                foreach (var process in Process.GetProcessesByName("BiglandsEngine.ConnectionRouter"))
                {
                    try
                    {
                ...
                // Try to locate using BiglandsEngine.ConnectionRouter package
                var logger = new LoggerResult();
                var package = PackageStore.Instance.FindLocalPackage("BiglandsEngine.ConnectionRouter", new PackageVersionRange(new PackageVersion(BiglandsEngineVersion.NuGetVersion)));
                if (package != null)
                {
                ...
                if (runningRouterVersion != null)
                {
                    ...
                    // Check that current router is at least as good as the one of latest found BiglandsEngine
