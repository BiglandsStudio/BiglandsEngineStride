...
using BiglandsEngine.Core.Assets.Yaml;
...
using BiglandsEngine.Core.Yaml;
using BiglandsEngine.Assets;
using BiglandsEngine.Graphics;
using BiglandsEngine.Core.ProjectTemplating;
using Project = BiglandsEngine.Core.VisualStudio.Project;
...
namespace BiglandsEngine.ProjectGenerator
{
    class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            var p = new OptionSet
                {
                    "Copyright (c) .NET Foundation and Contributors (https://dotnetfoundation.org/ & https://BiglandsEngine3d.net) and Silicon Studio Corp. (https://www.siliconstudio.co.jp) All Rights Reserved",
                    "BiglandsEngine Project Generator Tool - Version: "
                    +
                    String.Format(
                        "{0}.{1}.{2}",
            ...
            try
            {
                ...
                var templateFolder = @"..\..\sources\tools\BiglandsEngine.ProjectGenerator\Templates";
                ...
                switch (commandArgs[0])
                {
                    ...
                    case "project-unittest":
                        ...
                        GenerateUnitTestProject(
                            outputDirectory,
                            Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), Path.Combine(templateFolder, @"BiglandsEngine.UnitTests\BiglandsEngine.UnitTests.ttproj")),
        ...
        private static void GenerateUnitTestProject(string outputDirectory, string templateFile, string name, string projectNamespace)
        {
            ...
            // Force reference to BiglandsEngine.Assets (to have acess to SolutionPlatform)
            projectTemplate.Assemblies.Add(typeof(GraphicsProfile).Assembly.FullName);
            projectTemplate.Assemblies.Add(typeof(BiglandsEngineConfig).Assembly.FullName);
            ...
            // Compute BiglandsEngine Sdk relative path
            // We are supposed to be in standard output binary folder, so BiglandsEngine root should be at ..\..
            var BiglandsEnginePath = UPath.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), new UDirectory(@"..\.."));
            var BiglandsEngineRelativePath = new UDirectory(BiglandsEnginePath)
                .MakeRelative(outputDirectory)
                .ToString()
                .Replace('/', '\\');
            BiglandsEngineRelativePath = BiglandsEngineRelativePath.TrimEnd('\\');
            ...
            options["BiglandsEngineSdkRelativeDir"] = BiglandsEngineRelativePath;
        ...
        private static void SelectActiveProjects(Solution solution, string platform, List<ProjectProcessorContext> projectProcessorContexts, List<Project> removedProjects)
        {
            foreach (var solutionProject in solution.Projects.ToArray())
            {
                ...
                // Check BiglandsEngineSupportedPlatforms
                var buildTagsNode = doc.XPathSelectElement("/x:Project/x:PropertyGroup/x:BiglandsEngineBuildTags", mgr);
        ...
        private static void UpdateSolutionBuildConfigurations(string platform, Solution solution, List<ProjectProcessorContext> projectProcessorContexts)
        {
            ...
            if (configurations.Count > 0)
            {
                ...
                foreach (var context in projectProcessorContexts)
                {
                    ...
                    var BiglandsEnginePlatformNode =
                        context.Document.XPathSelectElement("/x:Project/x:PropertyGroup/x:BiglandsEnginePlatform",
                            context.NamespaceManager);

                    // Keep project platform only if BiglandsEnginePlatform is set manually to Windows and not a platform specific project
                    bool keepProjectPlatform = (BiglandsEnginePlatformNode != null && BiglandsEnginePlatformNode.Value == "Windows") && !context.Modified;
