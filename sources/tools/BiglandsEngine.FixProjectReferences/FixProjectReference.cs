...
namespace BiglandsEngine.FixProjectReferences
{
    public static class FixProjectReference
    {
            ...
            var p = new OptionSet
                {
                    ...
                    "BiglandsEngine Fix Project References - Version: "
                    +
                    String.Format(
                        "{0}.{1}.{2}",
        ...
        public static bool ProcessCopyLocals(ILogger log, string inputFile, bool isSavingMode)
        {
            ...
            foreach (var solutionProject in solution.Projects.ToArray())
            {
                ...
                var hasOutputPath = allElements.Any(element => element.Name.LocalName is "OutputPath" or "BiglandsEngineOutputPath");
                var isTest = allElements.Any(element => element.Name.LocalName == "BiglandsEngineOutputFolder" && element.Value.StartsWith("Tests", StringComparison.Ordinal));
