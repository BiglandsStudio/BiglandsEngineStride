...
#if BiglandsEngine_RUNTIME_NETFW
using System;
...
using BiglandsEngine.Assets.Templates;
using BiglandsEngine.Assets.Presentation.Templates;
...
namespace BiglandsEngine.VisualStudio.Package.Tests
{
    /// <summary>
    /// Test class that runs experimental instance of Visual Studio to check our plugin works well.
    /// </summary>
    /// <remarks>
    /// Right now it only has a test for .sdsl C# code generator, but it also tests a lot of things along the way: VSPackage properly have all dependencies (no missing .dll), IBiglandsEngineCommands can be properly found, etc...
    /// Also, it works against a dev version of BiglandsEngine, but it could eventually be improved to test against package version as well.
    /// </remarks>
    public class IntegrationTests : IDisposable
    {
        private const string StartArguments = @"/RootSuffix BiglandsEngine /resetsettings Profiles\General.vssettings";
        private DTE dte;
        ...
        public void TestXkslGeneration()
        {
            ...
            try
            {
                ...
                Assert.Equal("BiglandsEngineShaderKeyGenerator", sdslItem.Properties.Item("CustomTool").Value);
