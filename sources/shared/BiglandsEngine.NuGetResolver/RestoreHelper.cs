...
namespace BiglandsEngine.Core.Assets
{
    static class RestoreHelper
    {
            ...
            foreach (var lib in target.Libraries)
            {
                if (libPaths.TryGetValue(ValueTuple.Create(lib.Name, lib.Version), out var libPath))
                {
                    foreach (var a in lib.RuntimeAssemblies)
                    {
                        ...
                        // Check if BiglandsEngineGraphicsApi specific files exist
        ...
        public static (RestoreRequest, RestoreResult) Restore(ILogger logger, NuGetFramework nugetFramework, string runtimeIdentifier, string packageName, VersionRange versionRange, string settingsRoot = null)
        {
            ...
            var projectPath = Path.Combine("BiglandsEngineNugetResolver.json");
            var spec = new PackageSpec()
            {
                ...
                RestoreMetadata = new ProjectRestoreMetadata
                {
                    ...
                    OutputPath = Path.Combine(Path.GetTempPath(), $"BiglandsEngineNugetResolver-{packageName}-{versionRange.MinVersion.ToString()}-{nugetFramework.GetShortFolderName()}-{runtimeIdentifier}"),
            ...
            using (var context = new SourceCacheContext())
            {
                ...
                for (int tryCount = 0; tryCount < 2; ++tryCount)
                {
                    ...
                    {
                        // If we have an unauthorized access exception, it means assemblies are locked by running BiglandsEngine process
                        // During first try, kill some known harmless processes, and try again
                        if (tryCount == 1)
                            throw;

                        foreach (var process in new[] { "BiglandsEngine.ConnectionRouter", "BiglandsEngine.VisualStudio.Commands" }.SelectMany(Process.GetProcessesByName))
                        {
                            try
                            {
