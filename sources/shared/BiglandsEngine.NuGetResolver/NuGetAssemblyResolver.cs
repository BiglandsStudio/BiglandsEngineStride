...
namespace BiglandsEngine.Core.Assets
{
    ...
    {
        public const string DevSource = @"%LocalAppData%\BiglandsEngine\NugetDev";
        ...
        public static void SetupNuGet(string targetFramework, string packageName, string packageVersion)
        {
            ...
            string BiglandsEngineFolder = null;
            while (folder != null)
            {
                if (File.Exists(Path.Combine(folder, @"build\BiglandsEngine.sln")))
                {
                    BiglandsEngineFolder = folder;
                    var settings = NuGet.Configuration.Settings.LoadDefaultSettings(null);

                    Directory.CreateDirectory(Environment.ExpandEnvironmentVariables(DevSource));
                    CheckPackageSource(settings, "BiglandsEngine Dev", NuGet.Configuration.Settings.ApplyEnvironmentTransform(DevSource));
                ...
                if (!assembliesResolved)
                {
                    lock (assembliesLock)
                    {
...
#if BiglandsEngine_NUGET_RESOLVER_UI
                        ...
                        var newWindowThread = new Thread(() =>
                        {
                            ...
                            {
                                var splashScreen = new BiglandsEngine.NuGetResolver.SplashScreenWindow();
                                splashScreen.Show();
...
";
        ...
        /// <summary>
        /// Registers the listed native libs in BiglandsEngine.Core.NativeLibraryHelper using reflection to avoid a compile time dependency on BiglandsEngine.Core
        /// </summary>
        private static void RegisterNativeDependencies(Dictionary<string, string> assemblyNameToPath, List<string> nativeLibs)
        {
            var BiglandsEngineCoreAssembly = Assembly.LoadFrom(assemblyNameToPath["BiglandsEngine.Core"]);
            if (BiglandsEngineCoreAssembly is null)
                throw new InvalidOperationException($"Couldn't find assembly 'BiglandsEngine.Core' in restored packages");

            var nativeLibraryHelperType = BiglandsEngineCoreAssembly.GetType("BiglandsEngine.Core.NativeLibraryHelper");
            if (nativeLibraryHelperType is null)
                throw new InvalidOperationException($"Couldn't find type 'BiglandsEngine.Core.NativeLibraryHelper' in {BiglandsEngineCoreAssembly}");
