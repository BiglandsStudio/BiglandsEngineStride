...
using BiglandsEngine.Core.Presentation.Collections;
...
using BiglandsEngine.Core.VisualStudio;
using BiglandsEngine.LauncherApp.Resources;
using BiglandsEngine.LauncherApp.Services;
using BiglandsEngine.Metrics;
...
namespace BiglandsEngine.LauncherApp.ViewModels
{
    ...
    internal class LauncherViewModel : DispatcherViewModel, IPackagesLogger, IDisposable
    {
        ...
        private readonly SortedObservableCollection<BiglandsEngineVersionViewModel> BiglandsEngineVersions = new SortedObservableCollection<BiglandsEngineVersionViewModel>();
        ...
        private BiglandsEngineVersionViewModel activeVersion;
            ...
            : base(serviceProvider)
        {
            ...
            CheckDeprecatedSourcesCommand = new AnonymousTaskCommand(ServiceProvider, async () =>
            {
                ...
                if (NugetStore.CheckPackageSource(settings, "BiglandsEngine"))
                {
                    return;
                }
                // Add BiglandsEngine package store (still used for Xenko up to 3.0)
                ...
                {
                    NugetStore.UpdatePackageSource(settings, "BiglandsEngine", "https://packages.BiglandsEngine3d.net/nuget");
                    settings.SaveToDisk();
            ...
            {
                var version = new BiglandsEngineDevVersionViewModel(this, store, null, devVersion, false);
                BiglandsEngineVersions.Add(version);
            }
        ...
        public IEnumerable<BiglandsEngineVersionViewModel> BiglandsEngineVersions => BiglandsEngineVersions;
        ...
        public BiglandsEngineVersionViewModel ActiveVersion { get { return activeVersion; } set { SetValue(ref activeVersion, value); Dispatcher.InvokeAsync(() => StartStudioCommand.IsEnabled = (value != null) && value.CanStart); } }
        ...
        public ObservableList<DocumentationPageViewModel> ActiveDocumentationPages => ActiveVersion.Yield().Concat(BiglandsEngineVersions).OfType<BiglandsEngineStoreVersionViewModel>().FirstOrDefault()?.DocumentationPages;
        ...
        private async Task FetchOnlineData()
        {
            ...
            {
                await RetrieveLocalBiglandsEngineVersions();
...
```";
                ...
                await RetrieveServerBiglandsEngineVersions();
        ...
        public async Task RetrieveAllBiglandsEngineVersions()
        {
        ...
        private async Task FindReferencedPackages(NugetLocalPackage package)
        {
            foreach (var dependency in package.Dependencies)
            {
                ...
                if (prefix is not "BiglandsEngine" and not "Xenko")
                {
                    continue;
                }
        ...
        public async Task RetrieveLocalBiglandsEngineVersions()
        {
            ...
            try
            {
                var localPackages = await RunLockTask(() => store.GetPackagesInstalled(store.MainPackageIds).FilterBiglandsEngineMainPackages().OrderByDescending(p => p.Version).ToList());
                lock (objectLock)
                {
                    // Try to remove unused BiglandsEngine/Xenko packages after uninstall or update
                    try
                    {
...
```";
                    ...
                    var updatedLocalPackages = new HashSet<BiglandsEngineStoreVersionViewModel>();
                    foreach (var package in packages)
                    {
                        ...
                        if (localPackage != null)
                        {
                            ...
                            int index = BiglandsEngineVersions.BinarySearch(Tuple.Create(localPackage.Version.Version.Major, localPackage.Version.Version.Minor));
                            BiglandsEngineStoreVersionViewModel version;
                            if (index < 0)
                            {
                                // If not, add it
                                version = new BiglandsEngineStoreVersionViewModel(this, store, localPackage, localPackage.Id, localPackage.Version.Version.Major, localPackage.Version.Version.Minor);
                                Dispatcher.Invoke(() => BiglandsEngineVersions.Add(version));
                            }
                            else
                            {
                                version = (BiglandsEngineStoreVersionViewModel)BiglandsEngineVersions[index];
                            }
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        foreach (var BiglandsEngineUninstalledVersion in BiglandsEngineVersions.OfType<BiglandsEngineStoreVersionViewModel>().Where(x => !updatedLocalPackages.Contains(x)))
                            BiglandsEngineUninstalledVersion.UpdateLocalPackage(null, Array.Empty<NugetLocalPackage>());
                    });

                    // Update the active version if it is now invalid.
                    if (ActiveVersion == null || !BiglandsEngineVersions.Contains(ActiveVersion) || !ActiveVersion.CanDelete)
                        ActiveVersion = BiglandsEngineVersions.FirstOrDefault(x => x.CanDelete);

                    if (!lastActiveVersionRestored)
                    {
                        var restoredVersion = BiglandsEngineVersions.FirstOrDefault(x => x.CanDelete && x.Name == LauncherSettings.ActiveVersion);
                ...
                Dispatcher.Invoke(() => BiglandsEngineVersions.RemoveWhere(x => x is BiglandsEngineDevVersionViewModel));
                foreach (var package in devPackages)
                {
                    try
                    {
                        var realPath = store.GetRealPath(package);
                        var version = new BiglandsEngineDevVersionViewModel(this, store, package, realPath, true);
                        Dispatcher.Invoke(() => BiglandsEngineVersions.Add(version));
                    }
                    catch (Exception e)
                    {
            ...
            finally
            {
                Dispatcher.Invoke(() =>
                {
                    foreach (var project in currentRecentProjects)
                    {
                        ...
                        if (project.BiglandsEngineVersionName == "1.0")
                            continue;

                        project.CompatibleVersions.Clear();
                        foreach (var version in BiglandsEngineVersions)
                        {
                            // We suppose all dev versions are compatible with any project.
                            if (version is BiglandsEngineDevVersionViewModel)
                                project.CompatibleVersions.Add(version);

                            if (version is BiglandsEngineStoreVersionViewModel storeVersion && storeVersion.CanDelete)
                            {
                                // Discard the version that matches the recent project version
                                if (project.BiglandsEngineVersion == new Version(storeVersion.Version.Version.Major, storeVersion.Version.Version.Minor))
                                    continue;

                                // Discard the versions that are anterior to the recent project version
                                if (project.BiglandsEngineVersion > storeVersion.Version.Version)
                                    continue;
...
```";
                ...
                lock (objectLock)
                {
                    ...
                    foreach (var package in packages)
                    {
                        ...
                        {
                            // Find if we already have this package in our list
                            int index = BiglandsEngineVersions.BinarySearch(Tuple.Create(serverPackage.Version.Version.Major, serverPackage.Version.Version.Minor));
                            BiglandsEngineStoreVersionViewModel version;
                            if (index < 0)
                            {
                                // If not, add it
                                version = new BiglandsEngineStoreVersionViewModel(this, store, null, serverPackage.Id, serverPackage.Version.Version.Major, serverPackage.Version.Version.Minor);
                                Dispatcher.Invoke(() => BiglandsEngineVersions.Add(version));
                            }
                            else
                            {
            ...
            finally
            {
                Dispatcher.Invoke(() =>
                {
                    ...
                    var latestVersion = BiglandsEngineVersions.FirstOrDefault();
        ...
        public async Task CheckForFirstInstall()
        {
            ...
            {
                foreach (var version in BiglandsEngineVersions.OfType<BiglandsEngineStoreVersionViewModel>().Where(x => x.CanDelete))
                {
            ...
            bool firstInstall = BiglandsEngineVersions.All(x => !x.CanDelete) && BiglandsEngineVersions.Any(x => x.CanBeDownloaded);
            ...
            await Dispatcher.InvokeTask(async () =>
            {
                if (firstInstall)
                {
                    ...
                    {
                        var versionToInstall = BiglandsEngineVersions.First(x => x.CanBeDownloaded);
        ...
        public async Task StartStudio(string argument)
        {
            ...
            try
            {
                ...
                if (ActiveVersion is BiglandsEngineStoreVersionViewModel activeStoreVersion && activeStoreVersion.Version.Version < new Version(1, 2, 0, 0))
                {
                    metricsForEditorBefore120 = new MetricsClient(CommonApps.BiglandsEngineEditorAppId, versionOverride: activeStoreVersion.Version.ToString());
                }
        ...
        public static bool HasDoneTask(string taskName)
        {
            ...
            using (var subkey = localMachine32.OpenSubKey(@"SOFTWARE\BiglandsEngine\"))
            {
                if (subkey != null)
                {
        ...
        {
            var localMachine32 = RegistryKey.OpenBaseKey(RegistryHive.CurrentUser, RegistryView.Registry32);
            using (var subkey = localMachine32.CreateSubKey(@"SOFTWARE\BiglandsEngine\"))
            {
