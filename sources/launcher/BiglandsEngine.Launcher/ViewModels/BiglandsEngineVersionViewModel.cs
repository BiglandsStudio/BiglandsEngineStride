...
{
    /// <summary>
    /// An implementation of the <see cref="PackageVersionViewModel"/> that represents a major version of BiglandsEngine.
    /// </summary>
    internal abstract class BiglandsEngineVersionViewModel : PackageVersionViewModel, IComparable<BiglandsEngineVersionViewModel>, IComparable<Tuple<int, int>>
    {
        public const string MainExecutables = @"lib\net472\BiglandsEngine.GameStudio.exe,lib\net472\Xenko.GameStudio.exe,Bin\Windows\Xenko.GameStudio.exe,Bin\Windows-Direct3D11\Xenko.GameStudio.exe";
        private const string BiglandsEngineGameStudioExe = "BiglandsEngine.GameStudio.exe";
        ...
        internal BiglandsEngineVersionViewModel(LauncherViewModel launcher, NugetStore store, NugetLocalPackage localPackage, string packageId, int major, int minor)
        ...
        protected void UpdateFrameworks()
        {
            ...
            foreach (var toplevelFolder in new[] { "tools", "lib" })
            {
                ...
                if (Directory.Exists(libDirectory))
                {
                    ...
                    {
                        if (File.Exists(Path.Combine(frameworkPath, Major >= 4 ? BiglandsEngineGameStudioExe : XenkoGameStudioExe)))
                        {
        ...
        public string LocateMainExecutable()
        {
            ...
            if (SelectedFramework != null)
            {
                foreach (var toplevelFolder in new[] { "tools", "lib" })
                {
                    ...
                    foreach (var gameStudioExecutable in new[] { "BiglandsEngine.GameStudio.exe", "Xenko.GameStudio.exe" })
                    {
        ...
        public int CompareTo(BiglandsEngineVersionViewModel other)
        {
