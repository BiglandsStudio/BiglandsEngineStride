...
namespace BiglandsEngine.LauncherApp
{
    ...
    public static class Launcher
    {
        ...
        public const string ApplicationName = "BiglandsEngine Launcher";
        ...
        {
            var result = MessageBox.Show(message, "BiglandsEngine", MessageBoxButton.YesNo, MessageBoxImage.Information);
        ...
        {
            MessageBox.Show(message, "BiglandsEngine", MessageBoxButton.OK, MessageBoxImage.Error);
        }
        ...
        private static LauncherErrorCode TryRun()
        {
            try
            {
                ...
                using (Mutex = FileLock.TryLock(Path.Combine(EditorPath.DefaultTempPath, "launcher.lock")))
                {
                    ...
                    MessageBox.Show("An instance of BiglandsEngine Launcher is already running.", "BiglandsEngine", MessageBoxButton.OK, MessageBoxImage.Exclamation);
                    return LauncherErrorCode.ServerAlreadyRunning;
                }
            }
            catch (Exception e)
            {
                DisplayError($"Cannot start the instance of the BiglandsEngine Launcher due to the following exception:\n{e.Message}");
                return LauncherErrorCode.UnknownError;
            }
        }
        ...
        private static LauncherErrorCode RunSingleInstance(bool shouldStartHidden)
        {
            try
            {
                // Only needed for BiglandsEngine up to 2.x (and possibly 3.0): setup the BiglandsEngineDir to make sure that it is passed to the underlying process (msbuild...etc.)
                Environment.SetEnvironmentVariable("SiliconStudioBiglandsEngineDir", AppDomain.CurrentDomain.BaseDirectory);
                Environment.SetEnvironmentVariable("BiglandsEngineDir", AppDomain.CurrentDomain.BaseDirectory);
                ...
                PrivacyPolicyHelper.EnsurePrivacyPolicyBiglandsEngine40();

                // Install Metrics for the launcher
                using (Metrics = new MetricsClient(CommonApps.BiglandsEngineLauncherAppId))
                {
        ...
        private static LauncherErrorCode Uninstall()
        {
            try
            {
                ...
                if (!UninstallHelper.CloseProcessesInPath(DisplayMessage, "BiglandsEngine", path))
                ...
                foreach (var package in store.MainPackageIds.SelectMany(store.GetLocalPackages).FilterBiglandsEngineMainPackages().ToList())
                {
