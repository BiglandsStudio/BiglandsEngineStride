...
using BiglandsEngine.LauncherApp.Views;
using MessageBoxButton = BiglandsEngine.Core.Presentation.Services.MessageBoxButton;
using MessageBoxImage = BiglandsEngine.Core.Presentation.Services.MessageBoxImage;

namespace BiglandsEngine.LauncherApp.Services
{
    public static class SelfUpdater
    {
        ...
        internal static async Task DownloadAndInstallNewVersion(IDispatcherService dispatcher, IDialogService dialogService, string BiglandsEngineInstallerUrl)
        {
            try
            {
                ...
                var BiglandsEngineInstaller = Path.Combine(Path.GetTempPath(), $"BiglandsEngineSetup-{Guid.NewGuid()}.exe");
                using (var response = await httpClient.GetAsync(BiglandsEngineInstallerUrl))
                {
                    response.EnsureSuccessStatusCode();

                    await using var responseStream = await response.Content.ReadAsStreamAsync();
                    await using var fileStream = File.Create(BiglandsEngineInstaller);
                    responseStream.CopyTo(fileStream);
                }

                var startInfo = new ProcessStartInfo(BiglandsEngineInstaller)
                {
