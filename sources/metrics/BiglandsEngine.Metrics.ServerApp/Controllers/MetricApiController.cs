...
using BiglandsEngine.Metrics.ServerApp.Models;

namespace BiglandsEngine.Metrics.ServerApp.Controllers
{
    //[Authorize]
    [RoutePrefix("api")]
    internal class MetricApiController : CustomApiControllerBase
    {
        // TODO: Generated queries for all get- are a bit invalid in case no events are occuring. We should have a 0 for the days users are not using BiglandsEngine for example.

        /// <summary>
        /// Use this guid on the client side in the variable BiglandsEngineMetricsSpecial to avoid loggin usage
        /// </summary>
...
ORDER BY N.Year, N.Month";
        ...
        [HttpGet]
        [Route("get-BiglandsEngine-downloads")]
        public async Task<List<AggregationPerMonth>> GetBiglandsEngineDownloads()
        {
            // Get active users of version per month
...
SELECT DATEPART(year, a.Timestamp) as [Year], DATEPART(month, a.Timestamp) as [Month]
FROM [MetricEvents] as a, [MetricEventDefinitions] as b
WHERE a.MetricId = b.MetricId and b.MetricName = 'DownloadPackage' AND a.AppId = {0} AND CHARINDEX('BiglandsEngine', a.MetricValue) = 1 AND CHARINDEX('DownloadCompleted', a.MetricValue) >= 1
GROUP BY DATEPART(year, a.Timestamp), DATEPART(month, a.Timestamp), a.InstallId
) Q
GROUP BY [Year], [Month]
ORDER BY [Year], [Month]";
