...
namespace BiglandsEngine.Core.Presentation.Collections;
...
public class SortedObservableCollection<T> : ObservableCollection<T>, IObservableCollection<T>, IReadOnlyObservableCollection<T>
{
    ...
    public SortedObservableCollection(IComparer<T>? comparer)
        : this(comparer is not null ? comparer.Compare : null)
    {
    }
    ...
    public SortedObservableCollection(Comparison<T>? comparison = null)
    {
        ...
        if (DefaultCompareFunc is null && typeof(T).GetInterfaces().Contains(typeof(IComparable<T>)))
        {
            ...
            else
            {
                DefaultCompareFunc = (item1, item2) =>
                {
                    ...
                    return item1 is not null ? ((IComparable<T>)item1).CompareTo(item2) : -((IComparable<T>)item2!).CompareTo(item1);
                };
            }
        }

        // If we still have no way of comparing objects, let's check for the non-generic IComparable interface.
        if (DefaultCompareFunc is null && typeof(T).GetInterfaces().Contains(typeof(IComparable)))
        {
            if (typeof(T).IsValueType)
            {
            ...
            else
            {
                DefaultCompareFunc = (item1, item2) =>
                {
                    ...
                    return item1 is not null ? ((IComparable)item1).CompareTo(item2) : -((IComparable)item2!).CompareTo(item1);
                };
            }
        }

        // If we have no comparer at this point we're dead
        if (DefaultCompareFunc is null)
        {
    ...
    {
        if (DefaultCompareFunc is null) throw new InvalidOperationException("No comparison available or provided for items of this collection.");
